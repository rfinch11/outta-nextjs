...
async function scrapeEventPage(uid) {
  ...
  try {
    ...
    // Strategy 1: Look for event-description class
    const descriptionEl = $('.event-description, .description, [class*="description"]').first();
    description = descriptionEl.text().trim();

    // Strategy 2: If description is too short (< 50 chars), get the first substantial paragraph
    if (!description || description.length < 50) {
      $('p').each((i, el) => {
        const text = $(el).text().trim();
        // Look for paragraph with substantial content (> 100 chars) and event-related keywords
        if (text.length > 100 && text.match(/join|program|event|storytime|activity|designed|help|learn|create/i)) {
          description = text;
    ...
    // Strategy 3: Fallback to first paragraph if still no description
    if (!description || description.length < 50) {
      $('p').each((i, el) => {
        const text = $(el).text().trim();
        if (text.length > 50) {
...
async function importEvents() {
  ...
  // Process each event
  const testLimit = Math.min(3, icalEvents.length); for (let i = 0; i < testLimit; i++) {
